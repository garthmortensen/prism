...
def generate_permutations(op_name: str, config_cls: Type[Any]):
    ...
    for name, field in iterator:
        ...
        if is_enum(annotation):
            varying_fields[name] = get_enum_values(annotation)
        else:
    ...
    for idx, combo in enumerate(permutations):
        ...
        if config_dict.get("invalid_gender") == "error":
            continue
        ...
        # Create descriptive filename from varying fields
        if keys:
            parts = [f"{k}-{v}" for k, v in zip(keys, combo)]
            filename = f"{op_name}__" + "__".join(parts) + ".yaml"
        else:
        ...
        with open(filepath, "w") as f:
            f.write(f"# Auto-generated permutation {idx+1} for {op_name}\n")
